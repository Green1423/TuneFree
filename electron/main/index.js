...
log.transports.file.resolvePathFn = () =>
  join(app.getPath("documents"), "/TuneFree/TuneFree-log.txt");
// 设置日志文件的最大大小为 2 MB
...
class MainProcess {
  ...
  async init() {
    ...
    // 注册应用协议
    app.setAsDefaultProtocolClient("TuneFree");
    // 应用程序准备好之前注册
  ...
  showLyricWin() {
    if (this.lyricWindow) {
      ...
      // 生产模式
        ...
        this.lyricWindow.loadURL(
          `http://127.0.0.1:${import.meta.env.MAIN_VITE_MAIN_PORT ?? 7899}/#/lyric`,
        );
      }
      this.showLyricWin();
    }
  ...
  mainAppEvents() {
    app.whenReady().then(async () => {
      // 创建主窗口
      this.createWindow();
      this.downloadManager = new DownloadManager(app.getPath("exe"));
      // 检测更新
